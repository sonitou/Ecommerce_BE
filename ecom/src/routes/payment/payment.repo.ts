...
export class PaymentRepo {
  ...
  async receiver(body: WebhookPaymentBodyType): Promise<MessageResType> {
    ...
    // 2. Kiểm tra nội dung chuyển khoản và tổng số tiền khớp hay không
    ...
    await this.prismaService.$transaction([
      ...
      this.prismaService.order.updateMany({
        ...
        data: {
          status: OrderStatus.PENDING_PICKUP, // Assuming this is the next status after payment success
        },
      }),
    ])
