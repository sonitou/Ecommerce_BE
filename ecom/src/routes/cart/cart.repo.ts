...
export class CartRepo {
  ...
  private async validateSKU({
    ...
    if (!sku) {
      throw NotFoundSKUException // SKU không tồn tại
    }
    // Nếu người dùng thêm mới isCreate === true mà SKU đó đã có trong giỏ,
    // Và tổng số lượng cũ + mới > số lượng tồn kho hiện tại của SKU,
    if (cartItem && isCreate && quantity + cartItem.quantity > sku.stock) {
      throw InvalidQuantityException // vượt quá số lượng tồn kho
    }
    // Kiểm tra lượng hàng còn lại
    if (sku.stock < 1 || sku.stock < quantity) {
      throw OutOfStockSKUException // SKU đã hết hàng hoặc số lượng yêu cầu vượt quá số lượng tồn kho
    }
    ...
    if (
      ...
      throw ProductNotFoundException // không tìm thấy sản phẩm
    }
    return sku
  }
